...
- [x] ユーザー入力からスポット選択アルゴリズム ✅ (Gemini API)
- [x] ルート最適化（Gemini + Google Maps Grounding） ✅
- [x] レスポンス生成 ✅
- [x] テスト実装（手動検証済み） ✅
...
- [x] プラン取得後、地図画面に遷移 ✅ (SearchResultScreen)
- [x] `Polyline` でルート描画 ✅
- [x] 複数スポット間のルート表示 ✅
- [x] 地図の自動ズーム調整（全ルートが見える範囲） ✅
- [ ] ルート色・太さの調整
...
- [x] スポットごとにマーカー配置 ✅
- [x] マーカー番号表示（訪問順: 1→2→3...） ✅
- [x] マーカータップでスポット情報表示（Callout） ✅
  - スポット名、滞在時間、カテゴリ
- [ ] カスタムマーカーデザイン（オプション）
...
> [!NOTE]
> **2025/12/03 進捗メモ**:
> Phase 3 までほぼ完了。AI SDK も新バージョン (`@google/genai`) に移行済み。
> 次は Phase 3.5 (UX改善) と Phase 4 (データ永続化) に進みます。

---

## Phase 3.5: UX改善とクオリティ向上（現在進行中）

### 目標

MVPの使い勝手を向上させ、実用レベルに引き上げる

### 3.5-1. Google Maps 連携 ⭐⭐⭐

**優先度: 最高**

- [ ] 検索結果画面に「Google Mapsで開く」ボタンを追加 (Issue #35)
- [ ] ディープリンク (comgooglemaps:// または https://) の実装
- [ ] 経路案内モードでの起動

**理由**: アプリ内で完結しない詳細なナビゲーションをGoogle Mapsに委譲するため
**所要時間**: 1-2 時間
