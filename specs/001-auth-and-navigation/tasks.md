...
- [ ] T014a [P] AuthContextのユニットテストを作成（__tests__/unit/AuthContext.test.tsx）- セッション状態管理、ログイン/ログアウト処理、トークン検証ロジックをテスト
...
- AuthContextのユニットテストがすべてパスする（カバレッジ80%以上）
...
- [ ] T021a [P] [US1] LoginScreenコンポーネントのユニットテストを作成（__tests__/unit/LoginScreen.test.tsx）- UI表示、ボタンタップ、エラーメッセージ表示をテスト
...
- [ ] LoginScreenコンポーネントのユニットテストがすべてパスする（カバレッジ80%以上）
- [ ] 認証フローの統合テストがすべてパスする
...
- [ ] T030a [P] [US2] MapScreenとProfileScreenコンポーネントのユニットテストを作成（__tests__/unit/screens.test.tsx）- プレースホルダー表示、ユーザー情報表示、UI要素のレンダリングをテスト
- [ ] T031 [P] [US2] ナビゲーションロジックのユニットテストを作成（__tests__/unit/navigation.test.tsx）- 認証状態に基づく画面分岐、タブ切り替え、アクティブタブ表示をテスト
...
- [ ] タブ切り替え時間が500ms以内
- [ ] 画面コンポーネントのユニットテストがすべてパスする（カバレッジ80%以上）
- [ ] ナビゲーションロジックのユニットテストがすべてパスする（カバレッジ80%以上）
...
- [ ] 全ユニットテスト（AuthContext, LoginScreen, MapScreen, ProfileScreen, ナビゲーション）が通過
- [ ] 全統合テスト（認証フロー、セッション永続化、ログアウト）が通過
- [ ] テストカバレッジ80%以上達成（Constitution V準拠）
...
#### Phase 2（基盤実装）
- T012, T013, T014a（型定義とAuthContextユニットテスト）は並行実行可能

#### Phase 3（US1: ログイン）
- T021a（LoginScreenユニットテスト）はT016-T020完了後に並行実行可能
...
#### Phase 4（US2: ナビゲーション）
- T023, T024, T025, T026, T027, T028（ナビゲーション実装）は並行実装可能
- T030a, T031（画面コンポーネントとナビゲーションのユニットテスト）は並行実行可能
...
**総タスク数**: 55タスク（ユニットテスト強化により+3）

| Phase | タスク数 | 並行実行可能数 | 予想所要時間 |
|-------|---------|-------------|-----------|
| Phase 1: セットアップ | 10 | 6 | 1-1.5時間 |
| Phase 2: 基盤実装 | 6 | 3 | 1.5-2時間 |
| Phase 3: US1（ログイン） | 8 | 1 | 2.5-3.5時間 |
| Phase 4: US2（ナビゲーション） | 10 | 8 | 2.5-3.5時間 |
| Phase 5: US3（セッション永続化） | 7 | 0 | 1.5-2時間 |
| Phase 6: US4（ログアウト） | 6 | 0 | 1-1.5時間 |
| Phase 7: 仕上げ | 8 | 2 | 1.5-2時間 |

**合計予想時間**: 11-17時間（実働2-3日）

**並行実行機会**: 20タスク（全体の36%）

**ユニットテストカバレッジ目標**: 80%以上（Constitution V準拠）
...
✅ 依存関係が明確に文書化されている
✅ Constitution V準拠のため、ユニットテストタスク（T014a, T021a, T030a）を追加済み
✅ テストカバレッジ目標80%以上を各完了基準に明記
